class EventLogComponent < ViewComponent::Base
  def initialize(events:, endpoint:, path_builder:, dom_id:, key_prefix: "events")
    @events = events
    @endpoint = endpoint
  ...
  attr_reader :events, :endpoint, :path_builder, :dom_id, :key_prefix
  ...
  def refresh_button
    content_tag(:div, class: "flex justify-end") do
      button_tag(type: "button", class: "inline-flex items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-600 shadow-sm transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-1", data: { action: "polling#refresh", key: "#{key_prefix}.refresh" }) do
  ...
  def events_body
    if events.any?
      content_tag(:div, class: "space-y-3", data: { key: "#{key_prefix}.list" }) do
  ...
  def event_card(event)
    render CardComponent.new(href: path_builder.call(event), class: "p-4", data: { key: "#{key_prefix}.#{event.id}" }) do
      ...
      safe_join([
        content_tag(:div, class: "flex flex-wrap items-center gap-2") do
          safe_join([
            ...
            content_tag(:code, event.type, class: "text-sm font-semibold text-slate-800", data: { key: "#{key_prefix}.type" })
          ])
        end,
  ...
  def event_context(event)
    parts = []
    parts << user_label(event) if event.user_id.present? || admin_log?
  ...
  def user_label(event)
    if event.user_id.present?
      if admin_log?
        helpers.link_to("User ##{event.user_id}", helpers.admin_events_path(filter: { user_id: event.user_id }), class: "hover:text-slate-700", data: { key: "admin.events.user" })
      else
        helpers.tag.span("User ##{event.user_id}", data: { key: "#{key_prefix}.user" })
      end
    else
      helpers.tag.em("System", data: { key: "admin.events.user" })
    end
  end
  ...
  def subject_label(event)
    ...
    helpers.tag.span(value, data: { key: "#{key_prefix}.subject" })
  end

  def event_time(event)
    content_tag(:span, helpers.short_time_ago(event.created_at), class: "shrink-0 text-xs font-medium text-slate-500", title: event.created_at.rfc3339, data: { key: "#{key_prefix}.timestamp" })
  end
