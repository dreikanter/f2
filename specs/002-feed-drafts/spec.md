...
- **FR-023a**: The feeds-index summary line (today produced by `feed_summary_line` from `active_count` + `inactive_count`) MUST include drafts. The summary MUST report three buckets when applicable: active feeds, inactive feeds, drafts (with appropriate pluralization and the "X, Y and Z" joining style). The controller MUST add a `@draft_feed_count` (or equivalent) alongside the existing active/inactive counts. Rationale: "the state of the system should be very clear at any moment" — having drafts visible only as row badges but absent from the summary undercuts the feature's north star.
...
- **The analogous round-trip for Freefeed access tokens.** When a user has no active access tokens and tries to create a feed, today they hit `feeds/_blocked_no_tokens.html.erb`. Extending the draft round-trip to that gate is a sensible follow-up but is *not* in this spec. **Conscious trade-off**: leaving this out for now means two visually similar "you need credentials before continuing" surfaces with *opposite* save behavior — the AI-credential gate saves your work as a draft and round-trips you back; the Freefeed-token gate still dead-ends you. The inconsistency is real and user-facing, and is tracked separately as follow-up issue [#424](https://github.com/dreikanter/f2/issues/424).
