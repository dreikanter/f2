...
class AccessTokenListItemComponentTest < ViewComponent::TestCase
  def user
    @user ||= create(:user)
  end
  ...
  test "#render should use dom_id as element id" do
    ...
    assert_not_empty result.css("##{ActionView::RecordIdentifier.dom_id(access_token)}")
  end

  test "#render should render the token name as a link to the show page" do
    result = render_inline(AccessTokenListItemComponent.new(access_token: access_token))
  ...
  test "#render should show the active status icon" do
    result = render_inline(AccessTokenListItemComponent.new(access_token: access_token))

    icon = result.at_css("[data-key='access_token.#{access_token.id}.status_icon'] svg")
    assert_not_nil icon
    assert_equal "Active", icon["aria-label"]
  end

  test "#render should render the Edit menu item linking to the edit page" do
    result = render_inline(AccessTokenListItemComponent.new(access_token: access_token))
  ...
  test "#render should display owner and host domain" do
    result = render_inline(AccessTokenListItemComponent.new(access_token: access_token))

    assert_includes result.text, "@#{access_token.owner} at #{access_token.host_domain}"
  end

  test "#render should display host domain when owner is blank" do
    token = create(:access_token, user: user, owner: nil)
    result = render_inline(AccessTokenListItemComponent.new(access_token: token))

    assert_includes result.text, token.host_domain
    assert_not_includes result.text, "@"
  end

  test "#render should show 'Never used' when last_used_at is nil" do
    result = render_inline(AccessTokenListItemComponent.new(access_token: access_token))

    assert_includes result.text, "Never used"
  end

  test "#render should show last used time when available" do
  ...
  test "#render should set data-key attribute" do
    result = render_inline(AccessTokenListItemComponent.new(access_token: access_token))
