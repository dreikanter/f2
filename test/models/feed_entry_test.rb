...
class FeedEntryTest < ActiveSupport::TestCase
  ...
  test "#initialize should default status to pending" do
    entry = FeedEntry.new
    assert_equal "pending", entry.status
  end

  test "#status= should accept pending and processed statuses" do
    entry = valid_feed_entry
  ...
  test "#save! should persist raw_data as JSONB" do
  ...
  test "#valid? should allow nil values for optional fields" do
    entry = build(:feed_entry, published_at: nil, raw_data: nil)
    assert entry.valid?
  end

  test "#valid? should reject an empty uid" do
