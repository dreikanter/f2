...
class Registration::ConfirmationsControllerTest < ActionDispatch::IntegrationTest
  test "#new should show resend confirmation form" do
  ...
  test "#create should not send confirmation email for active user but show same message" do
  ...
  test "#create should not send confirmation email for nonexistent user but show same message" do
    assert_no_enqueued_emails do
  ...
  test "#create should normalize email for lookup" do
