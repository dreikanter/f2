...
class EmailUpdatesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should redirect to login when not authenticated" do
    patch email_update_url(1), params: { user: { email_address: "new@example.com" } }
  ...
  test "should request email confirmation for valid email change" do
    sign_in_user
    assert_emails 1 do
      patch email_update_url(1), params: { user: { email_address: "newemail@example.com" } }
    end
  ...
  test "should not allow duplicate email address" do
    ...
    patch email_update_url(1), params: { user: { email_address: "taken@example.com" } }
    assert_redirected_to profile_path
  ...
  test "should reject empty email" do
    sign_in_user
    patch email_update_url(1), params: { user: { email_address: "" } }
    assert_redirected_to profile_path
  ...
  test "should reject same email as current" do
    sign_in_user
    patch email_update_url(1), params: { user: { email_address: user.email_address } }
    assert_redirected_to profile_path
    assert_equal "Please enter a valid new email address.", flash[:alert]
  end
end
